...
import { ArrowUpRight, Check, Gauge, Info, KeyRound, Smartphone } from "lucide-react";
...
import type { WebsiteProject } from "@/content/projects";
...
/**
 * The case study for a shipped site — everything from the hero's headline
 * down to the build decisions. Unchanged from when this was the only kind
 * of case study there was; it moved out of page.tsx when the AI lanes
 * arrived, because the two bodies share a shell and almost nothing else.
 *
 * It takes a `WebsiteProject`, not a `Project`. That is the point of the
 * split: `liveDemoUrl`, `lighthouse` and `admin` are read unguarded here
 * and the type is what guarantees they exist.
 */
export function WebsiteCaseStudy({ project }: { project: WebsiteProject }) {
  return (
    <>
